...
public class lab5
{
  public static void main(String args[]) throws IOException
  {
    ...
    if (args.length == 1) {
      ...
      // set GHR if specified
      isInteractive = 0;
      GHRsize = Integer.parseInt(args[2]);
      if (!(GHRsize == 2 || GHRsize == 4 || GHRsize == 8)) {
        System.out.println("enter valid GHR of 2, 4, or 8");
        System.exit(1);
      }
    }
    else {
    ...
    // create GHR
    int[] myGHR = new int[GHRsize];

    // create prediction table 2^GHR big
    int[] predicttable = new int[(int)Math.pow(2, GHRsize)];

    // create new branch predictor object
    BranchPredictor branchpredict = new BranchPredictor(0, 0, myGHR, predicttable);

    // create emulator object
    Emulator emulator = new Emulator(0, new int[32], new int[8192], branchpredict);
    ...
    while (true) {
        ...
        if (command.equals("h")) {
          ...
          int[] newGHR = new int[GHRsize];
          int[] newpredicttable = new int[(int)Math.pow(2, GHRsize)];
          BranchPredictor newbranchpredict = new BranchPredictor(0, 0, newGHR, newpredicttable);
          emulator = new Emulator(0, newRegisters, newDataMem, newbranchpredict);
          System.out.println("Simulator reset\n");
        }
        else if (command.equals("o")) {
          // create new file called coordinates.csv
          FileWriter csvWriter = new FileWriter("coordinates.csv");
          int x = 0;
          int y = x + 1;
          // cycle through data memory until no longer reaching points
          while (y < 8192 && emulator.getDataMemI(x) != 0 && emulator.getDataMemI(y) != 0) {
            csvWriter.append(Integer.toString(emulator.getDataMemI(x)));
            csvWriter.append(", ");
            csvWriter.append(Integer.toString(emulator.getDataMemI(y)));
            csvWriter.append("\n");
            x = x + 2;
            y = y + 2;
          }
          ...
          double accuracy = 0.0;
          int mypredictions = ((BranchPredictor)emulator.getBP()).getPredict();
          int mycorrect = ((BranchPredictor)emulator.getBP()).getCorrect();
          DecimalFormat df = new DecimalFormat();
          df.setMaximumFractionDigits(2);
          accuracy = ((double)mycorrect / (double)mypredictions) * 100;
          System.out.println("");
          System.out.println("accuracy " + df.format(accuracy) + "% (" + mycorrect + " correct predictions, " + mypredictions + " predictions)");
          System.out.println("");
        }
  ...
  private static Emulator emulate(ArrayList<Instruction> instArray, Emulator emulator, int instructions, int isR) {
    ...
    while (instCount >= 1) {
      ...
      // addi
        ...
        emulator.setPC(emulator.getPC() + 1);
        // checking for increment of $s7 for figure.asm
        if (((instArray.get(emulator.getPC() - 1)).getInst()).equals("addi")) {
          if (((Immediate)instArray.get(emulator.getPC() - 1)).getRT() == 23 && ((Immediate)instArray.get(emulator.getPC() - 1)).getRS() == 23) {
            System.out.println("line: " + (emulator.getPC() - 1) + " $s7 value: " + emulator.getRegistersI(23));
          }
        }
      }
      // sll
      ...
      // beq
        ...
        int ghr1 = 0;
        ((BranchPredictor)emulator.getBP()).incrementPredict();
        // if equal branch is taken
        if (emulator.getRegistersI(((Immediate)instArray.get(emulator.getPC())).getRS()) == emulator.getRegistersI(((Immediate)instArray.get(emulator.getPC())).getRT())) {
          ...
          // shift GHR
          result1 = true;
          ghr1 = ((BranchPredictor)emulator.getBP()).numGHR();
          // check if branch predictor guessed true
          if (((BranchPredictor)emulator.getBP()).successfulPrediction(ghr1, result1)) {
            ((BranchPredictor)emulator.getBP()).incrementCorrect();
          }
          ((BranchPredictor)emulator.getBP()).shiftGHR(result1);
          // update prediction at ghr
          ((BranchPredictor)emulator.getBP()).updatePrediction(ghr1, result1);
        }
        // if not equal branch is not taken
        else {
          emulator.setPC(emulator.getPC() + 1);
          // shift GHR
          result1 = false;
      ...
      // bne
        ...
        int ghr2 = 0;
        ((BranchPredictor)emulator.getBP()).incrementPredict();
        // if not equal branch is taken
        if (emulator.getRegistersI(((Immediate)instArray.get(emulator.getPC())).getRS()) != emulator.getRegistersI(((Immediate)instArray.get(emulator.getPC())).getRT())) {
          emulator.setPC(emulator.getPC() + 1 + ((Immediate)instArray.get(emulator.getPC())).getIMM());
          // shift GHR
          result2 = true;
          ghr2 = ((BranchPredictor)emulator.getBP()).numGHR();
          // check if branch predictor guessed true
          if (((BranchPredictor)emulator.getBP()).successfulPrediction(ghr2, result2)) {
            ((BranchPredictor)emulator.getBP()).incrementCorrect();
          }
          ((BranchPredictor)emulator.getBP()).shiftGHR(result2);
          // update prediction at ghr
          ((BranchPredictor)emulator.getBP()).updatePrediction(ghr2, result2);
        }
        // if equal branch is not taken
        else {
          emulator.setPC(emulator.getPC() + 1);
          // shift GHR
          result2 = false;
